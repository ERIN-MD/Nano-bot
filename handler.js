...
—◉ *لو حابب تستأنف:* wa.me/+201151094460
╚═════════════════════════╝
               `.trim();
              m.reply(messageText);
...
export async function participantsUpdate({ id, participants, action }) {
  ...
  switch (action) {
     ...
     if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  pp = 'https://telegra.ph/file/97fca1d4f43a5881bfd06.jpg'; // Assign default image URL
                  ppgp = 'https://telegra.ph/file/97fca1d4f43a5881bfd06.jpg'; // Assign default image URL
                } finally {
                  ...
                  let welcomeApiUrl = `https://api.popcat.xyz/welcomecard?background=${encodeURIComponent(
                    'https://telegra.ph/file/b122b41d36befd67f12e1.jpg'
                  )}&text1=${encodeURIComponent(
                    await this.getName(user)
                  )}&text2=اهلا+بالعضو+الجديد&text3=عدد+اعضاء+الجروب+:+${encodeURIComponent(
                    nthMember.toString()
            ...
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  let text = ('غور يجي غيرك طابور, @user')
                    .replace('@user', '@' + user.split('@')[0]);
                  ...
                  let leaveApiUrl = `https://api.popcat.xyz/welcomecard?background=${encodeURIComponent(
                    'https://telegra.ph/file/8eb4bcca4388be7e68a2f.jpg'
                  )}&text1=${encodeURIComponent(
                    await this.getName(user)
                  )}&text2=غار+في+داهية&text3=عدد+اعضاء+الجروب+:+${encodeURIComponent(
                    nthMember.toString()
                  )}&avatar=${encodeURIComponent(pp)}`;
    ...
    case 'daradmin':
    case 'darpoder':
      text = (chat.sPromote || tradutor.texto3 || conn.spromote || '@user ```اصبح مشرف```');
    case 'demote':
    case 'quitarpoder':
    case 'quitaradmin':
      if (!text) {
        text = (chat.sDemote || tradutor.texto4 || conn.sdemote || '@user ```لم يعد مشرفا```');
      }
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await mconn.conn.reply(nk.from, `اهلا ي *@${nk.from.split('@')[0]}*, ${nk.isVideo ? 'videollamadas' : 'بوت نانو'} بـص انـت اتـصلت عـلي الـبوت فا هـتـتـحظـر دلـوقـتـي روح كلـم الـمطـور لـو رنـيـت بـالـغـلـط ⚡👾`, false, { mentions: [nk.from] });
        // let data = global.owner.filter(([id, isCreator]) => id && isCreator)
        // await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝐁𝐘┋❥ 𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓┋ . 👑;;;\nFN:𝐁𝐘┋❥ 𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓┋ . 👑\nORG:𝐁𝐘┋❥ 𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓┋ . 👑\nTITLE:\nitem1.TEL;waid=201151094460:+201151094460\nitem1.X-ABLabel:𝐁𝐘┋❥ 𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓┋ . 👑\n𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: '𝐁𝐘┋❥ 𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓┋ . 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] انضم - لجروب - الدعم', body: '✪┋𝐁𝐘┋❥ 𝐃.𝐂.𝐂 ➟ 𝐍𝐀𝐍𝐎-𝐁𝐎𝐓 ┋✪', thumbnail: imagen1, sourceUrl: 'https://chat.whatsapp.com/CoAUgy0624LCKP81rNiMcB'}}}}, aa);
